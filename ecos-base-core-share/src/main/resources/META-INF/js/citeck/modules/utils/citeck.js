...
define([
    ...
    Citeck.HTML5.supportedInputTypes = function () {
        ...
        let inputTypes = ["search", "number", "range", "color", "tel", "url", "email", "date", "month", "week", "time", "datetime", "datetime-local"],
            supportedInputTypes = [];
    ...
    let supportedInputTypes = Citeck.HTML5.supportedInputTypes();
    let inputTypes = ["search", "number", "range", "color", "tel", "url", "email", "date", "month", "week", "time", "datetime", "datetime-local"];
    for (let i in inputTypes) {
    ...
    Citeck.utils.eval = function (context, string) {
        let evaluatedContextFunction = function () {
            let evaluatedExpression = eval("(" + string + ")");
    ...
    Citeck.utils.formatUserName = function (user, plain_text) {
        let name = Alfresco.util.message("label.none");
        if (user) {
            if (user.firstName) {
                name = user.firstName;
        ...
        _response: function (response, requests) {
            ...
            var process = function (request) {
                var callback = request.callback,
                    ids = request.ids,
                    result = _.isArray(ids) ? _.map(ids, function (id) {
                        return results[id];
                    }) : results[ids];
    ...
    Citeck.UI.preview = function (id, params) {
        if (!id) {
            throw Error("Argument 'id' should by defined");
            return;
        }
        if (!params.nodeRef) {
            throw Error("Parameter 'nodeRef' should by defined");
            return;
        }
    ...
    Citeck.UI.previewDependencies = function () {
        var sources = [
                ...
                "res/components/preview/PdfJs.js",
                "res/components/preview/pdfjs/compatibility.js",
                "res/components/preview/pdfjs/pdf.js",
                "res/components/preview/pdfjs/pdf.worker.js",
                "res/components/preview/spin.js",
    ...
    Citeck.UI.orgstruct = function (id, params) {
            ...
            buttons: [
                {
                    ...
                    handler: function (event) {
                        ...
                        else if (params.callback.ok instanceof Function) {
                            params.callback.ok.call(self, event);
                        }
                    }
                },
                {
                    ...
                    handler: function (e) {
                        ...
                        else if (params.callback.cancel instanceof Function) {
                            params.callback.cancel.call(self, event);
                        }
                    }
                }
            ]
        ...
        this.widgets.resize.on('resize', function (ev) {
            $("#" + secondPanel).width(800 - ev.width - 10);
        });
        ...
        YAHOO.util.Event.addListener(secondPanel, "click", function (event) {
            if (event.target.tagName == "LI") {
                ...
                if (node) {
                    $("table", node.getEl()).first().removeClass("selected unselectable").addClass("selectable")
                }
                ;
                $(event.target).remove();
            }
