...
function(item)
{
   ...
   <#-- explicit allow*Action parameters -->
   <#if field.control.params.allowAddAction??>
      <#local allowAddAction = (field.control.params.allowAddAction?string == "true") />
   </#if>
   <#if field.control.params.allowRemoveAllAction??>
      <#local allowRemoveAllAction = (field.control.params.allowRemoveAllAction?string == "true") />
   </#if>
   <#if field.control.params.allowRemoveAction??>
      <#local allowRemoveAction = (field.control.params.allowRemoveAction?string == "true") />
   </#if>
   <#if field.control.params.allowViewMoreAction??>
      <#local allowViewMoreAction = (field.control.params.allowViewMoreAction?string != "false") />
      <#local viewMoreAction = { "name": "view_more_actions", "label": "form.control.object-picker.workflow.view_more_actions", "link": documentLinkResolver }>
      <#if allowViewMoreAction>
         <#local actions = [ viewMoreAction ] />
      <#else />
         <#local actions = [] />
      </#if>
   </#if>
