...
<@markup id="js">
   ...
   <@script src="${url.context}/res/components/console/citeck-console-application.js" group="console"/>
</@>
...
<@markup id="html">
   <@uniqueIdDiv>
      ...
      <#assign defaultMobilelogo=msg("header.mobile-logo")><#if defaultMobilelogo="header.mobile-logo"><#assign defaultMobilelogo="app-logo-mobile.png"></#if>
         ...
         <div id="${el}-options" class="hidden">
            ...
            <form id="${el}-options-form" action="${url.context}/service/components/console/application" method="post">
               ...
               <div class="row">
                  ...
                  <div>
                     <button id="${el}-upload-main-logo-button" name="upload">${msg("button.upload")}</button>&nbsp;
                     <button id="${el}-reset-main-logo-button" name="reset">${msg("button.reset")}</button>
                     <div class="logonote">${msg("label.main-logo-note")}</div>
                     <input type="hidden" id="console-options-logo" value="" />
                  </div>
               </div>
               </@>
               <!-- Mobile Logo -->
               <@markup id="mobileLogo">
                  <div class="row">
                     <div class="label">${msg("label.mobile-logo")}:</div>
                     <div class="logo"><img id="${el}-mobileLogoImg" src="${url.context}<#if mobileLogo?? && mobileLogo?length!=0>/proxy/alfresco/api/node/${mobileLogo?replace('://','/')}/content<#else>/res/themes/${theme}/images/${defaultMobilelogo}</#if>" /></div>
                     <div>
                        <button id="${el}-upload-mobile-logo-button" name="uploadMobile">${msg("button.upload")}</button>&nbsp;
                        <button id="${el}-reset-mobile-logo-button" name="resetMobile">${msg("button.reset")}</button>
                        <div class="logonote">${msg("label.mobile-logo-note")}</div>
                        <input type="hidden" id="console-options-mobile-logo" value="" />
                     </div>
                  </div>
               </@>
               <!-- Apply changes -->
