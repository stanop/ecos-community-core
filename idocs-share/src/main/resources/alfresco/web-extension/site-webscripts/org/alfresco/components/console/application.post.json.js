...
function main()
{
   if (user.isAdmin)
   {
      ...
      if (logoId != null && (logoId = new String(logoId)).length != 0) {
       ...
       // mobile logo option
       var mobileLogoId = json.get("console-options-mobile-logo");
       if (mobileLogoId != null && (mobileLogoId = new String(mobileLogoId)).length != 0) {
           // "reset" is special case to reset application logo to theme default
           sc.setProperty("mobileLogo", (mobileLogoId != "reset" ? mobileLogoId : ""));
           sc.save();
       }
   }
   model.success = true;
