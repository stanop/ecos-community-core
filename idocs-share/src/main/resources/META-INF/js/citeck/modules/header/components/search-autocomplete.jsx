        ...
        documentsSection = (
            <section className='autocomplete-section'>
                ...
                <ul className='autocomplete-section__list'>
                    {documents.items.map(item => <SearchAutocompleteItem item={item} type={SEARCH_AUTOCOMPLETE_TYPE_DOCUMENTS} />)}
                </ul>
                {moreButtom}
            </section>
        );
    ...
    if (sites.items.length > 0) {
        sitesSection = (
            <section className='autocomplete-section'>
                ...
                <ul className='autocomplete-section__list'>
                    {sites.items.map(item => <SearchAutocompleteItem item={item} type={SEARCH_AUTOCOMPLETE_TYPE_SITES} />)}
                </ul>
            </section>
        );
    }
    ...
    if (people.items.length > 0) {
        peopleSection = (
            <section className='autocomplete-section'>
                ...
                <ul className='autocomplete-section__list'>
                    {people.items.map(item => <SearchAutocompleteItem item={item} type={SEARCH_AUTOCOMPLETE_TYPE_PEOPLE} />)}
                </ul>
            </section>
        );
    }
