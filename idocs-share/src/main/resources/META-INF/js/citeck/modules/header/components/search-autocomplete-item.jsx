...
const SearchAutocompleteItem = ({ type, item }) => {
    ...
    switch (type) {
        case SEARCH_AUTOCOMPLETE_TYPE_DOCUMENTS:
            ...
            thumbnailUrl = Alfresco.constants.PROXY_URI + 'api/node/' + item.nodeRef.replace(':/', '') + '/content/thumbnails/doclib?c=queue&ph=true&lastModified=' + lastModified;
            ...
            if (item.site) {
                ...
                siteDocLib = <Fragment><a href={siteDocLibUrl}>{item.site.title}</a> |</Fragment>;
            }
            ...
            itemMeta = <Fragment>{siteDocLib} <a href={modifiedUserUrl}>{item.modifiedBy}</a> | <span title={modifiedTimeParts.formatted}>{modifiedTimeParts.relative}</span> | {fileSize}</Fragment>;
            break;
