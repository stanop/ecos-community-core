        ...
        if (YAHOO.lang.isObject(nodeRef)) {
          if (this.options.onsubmit) {
            if (this.options.redirect) {
              var templateRegExp = new RegExp("{{nodeRef}}", "g");
              document.location.href = this.options.onsubmit.replace(templateRegExp, nodeRef.toString());
            } else if (this.options.onsubmit == "back") {
              if (history.length > 1) history.go(-1);
            } else if (this.options.onsubmit == "card") {
