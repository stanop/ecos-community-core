  ...
  YAHOO.extend(Alfresco.dashlet.RecentComments, Alfresco.component.Base, {
      ...
      else
      {
        ...
        for (i = 0, ii = comments.length; i < ii; i++)
        {
          ...
          if (this.options.rangeFilter == "today") {
            ...
            html += this.msg("") + Alfresco.util.formatDate(Alfresco.thirdparty.fromISO8601(comment.createdOn), this.msg("comments.date-format.default"));
          }

          if (comment.site && (this.options.siteId != comment.site.shortName))
          {
            html += " " + this.msg("message.in.site") +
            '<a href="' + $siteURL("dashboard", { site: comment.site.shortName }) + '" class="theme-color-1">' +
        ...
        this.footer.innerHTML = this.msg("message.footer.comments", comments.length);
    ...
    getCommentIcon: function RecentComments_getCommentIcon(comment)
    {
      ...
      case "blog":
        ...
        iconUrl += Alfresco.constants.URL_RESCONTEXT + 'components/images/blogpost-32.png';
        break;

      case "link":
        alt = comment.parent.title;
        iconUrl += Alfresco.constants.URL_RESCONTEXT + 'components/images/link-32.png';
        break;

      case "folder":
        alt = comment.parent.name;
        iconUrl += Alfresco.constants.URL_RESCONTEXT + 'components/images/filetypes/generic-folder-32.png';
        break;
