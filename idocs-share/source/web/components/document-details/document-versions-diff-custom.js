...
function getSignInfo(nodeRef){
    ...
    var dataNeed = eval('(' + xmlHttp_add.responseText + ')').data;

    var xmlHttp_add = new XMLHttpRequest();
    xmlHttp_add.open("GET", Alfresco.constants.PROXY_URI + "acm/digitalSignature?nodeRef=" + nodeRef, false);
    xmlHttp_add.send(null);
    if (xmlHttp_add.status != 200) {
        return;
    }
    var sign = eval('(' + xmlHttp_add.responseText + ')').data;

    try {
        var xmlHttp_add = new XMLHttpRequest();
        xmlHttp_add.open("POST", Alfresco.constants.PROXY_URI + "acm/checkValid", false);
        xmlHttp_add.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlHttp_add.send(
            "nodeRef=" + encodeURIComponent(nodeRef) +
            "&sign=" + encodeURIComponent(sign[0].sign) +
            "&data=" + dataNeed[0].result
        );
        if (xmlHttp_add.status != 200) {
            alert(xmlHttp_add.status);
        }
        var n = eval('(' + xmlHttp_add.responseText + ')').data;
    } catch(e){
        alert(e);
    }
    var info = [];
    info[0] = n;
    info[1] = sign[0].signer;
    return info;
}

function hasAspect(nodeRef, aspectName) {
	var xmlHttp_add = new XMLHttpRequest();
	xmlHttp_add.open("GET", Alfresco.constants.PROXY_URI + "acm/getDocumentAspects?nodeRef=" + nodeRef, false);
	xmlHttp_add.send(null);
	if (xmlHttp_add.status != 200) {
		return;
	}
	var aspects = eval('(' + xmlHttp_add.responseText + ')').data;
	for(var iter in aspects) {
        if(!aspects.hasOwnProperty(iter)) continue;
		if(aspects[iter] === aspectName) {
			return true;
		}
	}
	return false;
...
(function() {
   ...
   YAHOO.extend(Alfresco.DocumentVersions, Alfresco.component.Base,
   {
      ...
      options:
      {
         ...
         /**
          * Head document's version number
         ...
         this.widgets.alfrescoDataTable = new Alfresco.util.DataTable(
         {
            dataSource:
            {
               ...
               doBeforeParseData: this.bind(function(oRequest, oFullResponse)
               {
                  ...
                  $.fn.extVersionsController('init', { documentName: this['latestVersion']['name'] });
      ...
      getDocumentVersionMarkup: function DocumentVersions_getDocumentVersionMarkup(doc)
      {
    	 ...
    	 var versionsDiffURL = Alfresco.util.siteURL('versions-difference?nodeRef=' + this.options.nodeRef + '&amp;versRef=' + doc.nodeRef);
         ...
         html += $.fn.extVersionsController('generateSelectorHTML', {
              context: this.id,
              document: doc,
	     ...
	     //if (this.options.flag.length!=0) {
         if (hasAspect(this.options.nodeRef, "signable")) {
	         var info = getSignInfo(doc.nodeRef);
	         if (info[0] == true && info[1] != '') {
	             html +=' <img src="'+ Alfresco.constants.URL_RESCONTEXT +'components/document-details/images/status-COMPLETED.png" title="' + info[1] + '"/> ';
	         } else {
	             html +=' <img src="'+ Alfresco.constants.URL_RESCONTEXT +'components/document-details/images/status-FAILED.png" title="' + this.msg("status.notSign") + '"/> ';
	         }
         }
         ...
         html += '<div class="clear"></div>';
         this.options.flag = "asdad";
         return html;
      },

      addToCompare: function DocumentVersions_addToCompare(version) {
          $.fn.extVersionsController('addToCompare', {
              version: version
          });
      },
