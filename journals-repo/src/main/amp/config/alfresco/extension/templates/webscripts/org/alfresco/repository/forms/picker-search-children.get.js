<import resource="classpath:/alfresco/extension/templates/webscripts/org/alfresco/repository/forms/pickerresults.lib.js">
...
function main() {
    ...
    try {
        ...
        if (url.templateArgs.type == "node") {
            ...
            var childNodes = null;
            if (argsSelectableType) {
                var q = '+TYPE: "' + argsSelectableType + '"';
                var siteId = url.templateArgs.site_id;
                if (siteId && 'undefined' != siteId) {
                    q += ' +PATH:"/app:company_home/st:sites/cm:' + search.ISO9075Encode(siteId) + '//*"';
                }
                childNodes = search.query({
					...
					sort: [
						{
							column: "@" + utils.longQName("cm:name"),
							ascending: true
						}
					]
                });
            ...
            for each (var result in childNodes) {
                if (result.isContainer || result.type == "{http://www.alfresco.org/model/application/1.0}folderlink") {
                    ...
                    if (argsSearchTerm != null && filter == null) {
                        filter = new Object();
                        var filterVals = argsSearchTerm.split('!_!');
                        for (var i = 0; i < filterVals.length - 1; i = i + 2) {
                            logger.log(filterVals[i] + '->' + filterVals[i + 1])
                            if (filterVals[i + 1])
                                filter[filterVals[i]] = new RegExp(filterVals[i + 1], "gi");
                        }
                    }
                    if (argsSelectableType == "clarumdms:contractor") {
                        resultObj = {
                            item:createContractorResult(result)
                        };
                    } else {
                        resultObj = {
                            item:result
                    ...
                    if (filter)
                        for (var key in filter) {
                            filter[key].lastIndex = 0;
                            if (!filter[key].test(resultObj.item[key])) {
                                needToAdd = false;
                                break;
                            }
                        }
