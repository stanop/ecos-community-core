<#macro classDefJSON classdef key>
    ...
    {
		<#if classdef.name??>"name": "${classdef.name}",
	"prefixedName": "${classdef.name.toPrefixString()}",</#if>
...
<#macro classDefJSONForSelectInput classdef key>
    <#local classdefprefix=classdef.name.toPrefixString()?replace(":","_")>
    <#escape x as jsonUtils.encodeJSONString(x)>
        {
            "name": "${classdef.name}",
            "prefixedName": "${classdef.name.toPrefixString()}",
            "title": <#if (classdef.title)??>"${classdef.title}"<#else>null</#if>,
            "isAspect": ${classdef.isAspect()?string}
        }
    </#escape>
</#macro>