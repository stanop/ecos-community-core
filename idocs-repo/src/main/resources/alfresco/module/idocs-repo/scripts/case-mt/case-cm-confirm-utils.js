...
/*        var templateName = "cm:notification-template.html.ftl";
    ...
    canAddPerformer: function () {
        return caseRoleService.isRoleMember(document, ROLE_INITIATOR, person)
            || caseRoleService.isRoleMember(document, ROLE_CONFIRMERS, person)
            || caseRoleService.isRoleMember(document, ROLE_SIGNER, person)
            || caseRoleService.isRoleMember(document, ROLE_OWNER, person);
    },

    addAdditionalPerformers: function () {
        if (!this.canAddPerformer()) {
            return;
        }
        var performers = additionalData.assocs['iEvent:performers'];
        if (!performers.length) throw "Исполнители не выбраны";
        caseRoleService.addAssignees(document, ROLE_PERFORMERS, performers);
    }
};