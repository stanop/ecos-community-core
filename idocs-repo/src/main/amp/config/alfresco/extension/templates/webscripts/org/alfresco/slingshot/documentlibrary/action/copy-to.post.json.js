   ...
   for (file in files)
   {
      ...
      try
      {
         ...
         else
         {
            ...
            var isCustomCopy = copyToActionService.evaluate(nodeRef, destNode.getNodeRef());
            if (isCustomCopy && "false" != isCustomCopy) {
                var copiedNodeRef = copyToActionService.copy(nodeRef, destNode.getNodeRef());
                copiedNode = search.findNode(copiedNodeRef);
                result.nodeRef = copiedNode.getNodeRef().toString();
            } else if (fileNode.isContainer) {
