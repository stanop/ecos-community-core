    ...
    Alfresco.ActivitiTransitions.prototype._generateTransitionButton = function (transition) {
        ...
        var button = Alfresco.util.createYUIButton(this, transition.id, function (e, p_obj) {
            ...
            if (transition.id == "Complete") {
                var searchUrl = Alfresco.constants.PROXY_URI + 'citeck/get-document-package?nodeRef=' + nodeRef;
            ...
            } else if (transition.id == "Reject") {
                var comment = encodeURIComponent($('#${args.htmlid?html}_prop_bpm_comment')[0].value);
                var searchUrl = Alfresco.constants.PROXY_URI + 'citeck/get-documents-to-sign-reject?nodeRef=' + nodeRef + '&comment=' + comment;
                var request = new XMLHttpRequest();
                request.open('GET', searchUrl, false);
                request.send(null);
                if (request.status === 200) {
